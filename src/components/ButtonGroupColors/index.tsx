...
export function ButtonGroupColors({group, onSelect} : Props) {
    ...
    return (
        <View style={styles.buttonGroupContainer}>
            {group.map((item) => (<TouchableOpacity key={item.id} onPress={() => {handleSelect(item)}} style={[styles.buttonGroupButtonContainer, selected?.id === item.id && {borderColor: item.colorCode }]}>
                <View style={[styles.buttonGoupButtonColorIndicator, {backgroundColor: item.colorCode}]} />
